...
public class OrderProcessorFacade extends Middleware {
	...
	public void process() {
		...
		int orderedQnty = order.getOrderedQuantity();
		
		// Step 2 - compare the ordered quantity against the available and target max quantity for the product
		int stateNum = this.orderController.compareOrderedQntyAgainstProduct(orderedProduct, orderedQnty);
